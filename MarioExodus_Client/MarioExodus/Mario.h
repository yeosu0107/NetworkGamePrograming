...
public:
	...
	static enum MarioJumpState		{ Jump_None, Jump_Up, Jump_Down };								// - 클라
...
private:
	...
	float			m_fTimePerAction;		// 시간당 스프라이트 변화 속도							- 클라
	float			m_fActionTime;			// 해당 스프라이트로 바뀐 뒤 경과 시간					- 클라

	bool			m_bSelect;				// 마리오 선택 여부									- 서버, 클라
	bool			m_bLookDirection;		// 마리오 방향 true = 오른쪽, false = 왼쪽			- 서버, 클라
	bool			m_bExit;				// 마리오가 문밖을 나간 경우
	bool			m_bCurJump;				// 현재 점프 여부
	bool			m_bPreJump;				// 이전 프레임 점프 여부
